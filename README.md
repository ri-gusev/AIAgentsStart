...
- `validation`: кнопка «Проверить» сохраняет переход, добавляет отдельное сообщение состояния validation и сразу запускает LLM-проверку. Если проверка не пройдена, задача остаётся в validation, показывает отчёт и кнопку «Вернуться к execution». Эта кнопка автоматически переделывает работу по отчёту; текстовое замечание пользователя в validation также возвращает задачу в execution и сразу применяет указанную правку.
...
Validation не запускает команды операционной системы: у Agent нет tools. Он оценивает только доступные в контексте результаты и не должен выдумывать прохождение тестов.
