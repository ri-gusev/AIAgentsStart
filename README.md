...
- task state machine ведёт проект по этапам `planning → execution → validation → DONE` и сохраняет каждый переход в SQLite;
...
3. Если это первое сообщение нового чата, Agent всегда обрабатывает его как постановку задачи в `planning` и формирует план. Пока план не утверждён, следующее текстовое сообщение считается замечанием к плану и создаёт его новую версию.
...
- `planning`: первое сообщение нового чата всегда становится постановкой задачи. Agent отвечает планом прямо в ленте чата; под ним доступны «Утвердить план» и «Переделать план». Любое отправленное текстовое замечание до утверждения также перерабатывает план.
- `execution`: кнопка «Утвердить план» запускает отдельный LLM-вызов выполнения утверждённого плана. Agent добавляет результат новым сообщением с состоянием execution; под ним доступна кнопка «Проверить». Новое текстовое сообщение пользователя считается правкой к текущей работе и вызывает обновлённый execution-результат с учётом всего контекста задачи.
- `validation`: кнопка «Проверить» сохраняет переход, добавляет отдельное сообщение состояния validation и сразу запускает статическую LLM-проверку последнего результата. Validation не компилирует и не запускает файлы и не требует компиляцию как условие успеха. Если найдены содержательные проблемы, отчёт содержит конкретные инструкции для следующего execution. Кнопка «Вернуться к execution» применяет эти инструкции, после чего результат можно снова отправить в validation. Предыдущий отчёт сохраняется до завершения повторной проверки, чтобы этапы оставались явно связанными.
- `DONE`: при успешной проверке Agent добавляет новое сообщение с отчётом и состоянием DONE. При неуспехе пользователь явно возвращает задачу в execution отдельной кнопкой; этот переход также появляется новым сообщением.

Текущее состояние всегда заметно показано в цветной панели над лентой чата. Цвет, название и пояснение панели обновляются после каждого перехода.

Кнопки переходов находятся под последним ответом Agent в ленте чата, а не в отдельном sidebar-блоке. Каждый переход состояния сразу записывается в `project_task_states`. Pause не меняет этап, но атомарно сохраняет `paused=1` и новый `project_summaries.summary`, сформированный из предыдущего project summary, плана, validation report, рабочей памяти, conversation summary и всех raw-сообщений, ещё находящихся в RAM. Resume записывает `paused=0` без нового LLM-вызова и не изменяет summary. Пока проект на паузе, отправка сообщений и переходы этапов запрещены.
...
Схема расширена без удаления существующих данных: дополнительно создаются `chats`, `working_memory`, `project_summaries`, `project_task_states` и `agent_settings`. Для существующих чатов недостающие проектные строки автоматически создаются с пустым summary и состоянием `planning`. Старые факты в `long_term_memory` остаются общими.
...
Переключателя Auto/Manual и кнопок выбора памяти под сообщениями в интерфейсе нет. Под ответом Agent находятся только действия текущего task state: утверждение или переработка плана, переход к validation, запуск проверки, Pause и Resume. Панели рабочей и общей долговременной памяти справа показывают фактически сохранённые данные.
...
| `POST /api/chat` | `{"chat_id":"1","message":"Создай приложение на C++17 и SQLite"}` | Первым сообщением сформировать planning; затем переработать план или продолжить выполнение |
...
| `POST /api/task/approve` | `{"chat_id":"1"}` | Перейти `planning → execution` |
| `POST /api/task/validation` | `{"chat_id":"1"}` | Перейти `execution → validation`, вывести новое сообщение и сразу проверить результат |
| `POST /api/task/validate` | `{"chat_id":"1"}` | Повторить проверку, если предыдущий validation-вызов завершился технической ошибкой |
| `POST /api/task/execution` | `{"chat_id":"1"}` | После неуспешной проверки явно вернуть задачу `validation → execution` |
| `POST /api/task/pause` | `{"chat_id":"1"}` | Сжать проект, сохранить summary и поставить на паузу |
| `POST /api/task/resume` | `{"chat_id":"1"}` | Снять паузу без изменения summary |
...
- `task_state`: этап, план, validation report и признак Pause выбранного проекта;
